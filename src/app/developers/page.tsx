...
import { DeveloperDetails, PaginatedResponse } from "@/@types/types";

const API_KEY = process.env.NEXT_PUBLIC_RAWG_API_KEY;
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL;

export default function DevelopersPage() {
  const [developers, setDevelopers] = useState<DeveloperDetails[]>([]);
    ...
    async function fetchDevelopers() {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/developers?key=${API_KEY}&page=${currentPage}&page_size=${pageSize}`
        );
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.detail ||
              `Failed to fetch developers: ${response.statusText} (${response.status})`
          );
        }
        const data: PaginatedResponse<DeveloperDetails> = await response.json();
        setDevelopers(data.results);
        setTotalDevelopers(data.count);
        setTotalPages(Math.ceil(data.count / pageSize));
      } catch (err) {
        if (err instanceof Error) setError(err.message);
        else setError("An unknown error occurred while fetching developers.");
        setDevelopers([]);
      } finally {
        setLoading(false);
      }
  ...
  if (loading && developers.length === 0)
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-xl text-sky-400">Loading Game Developers...</p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 py-8 px-4 sm:px-6 lg:px-8">
      <h1 className="text-4xl sm:text-5xl font-extrabold mb-10 text-center">
        <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 py-1">
          Game Developers
        </span>
      </h1>
      ...
      {developers.length > 0 ? (
          ...
          {developers.map((dev) => (
            <Link
              ...
              className="
                ...
                hover:scale-105 hover:shadow-purple-500/30 hover:border-purple-600
            ...
            >
              ...
              <div className="p-4">
                <h3 className="text-lg sm:text-xl font-semibold mb-1 truncate text-slate-100 group-hover:text-purple-300 transition-colors">
                  {dev.name}
                </h3>
                <p className="text-sm text-slate-400 flex items-center">
                  <Gamepad2 size={14} className="mr-1.5 text-slate-500" />
                  Games: {dev.games_count.toLocaleString()}
                </p>
              </div>
            </Link>
          ))}
        ...
        !loading && (
          <div className="text-center py-20">
            ...
            <p className="text-xl text-slate-400">No developers found.</p>
      ...
      <p className="text-center mt-10 text-sm text-slate-400">
        {totalDevelopers > 0 &&
          `Showing ${
            developers.length > 0 ? (currentPage - 1) * pageSize + 1 : 0
          }-${Math.min(
