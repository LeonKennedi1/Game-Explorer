...
import { PublisherDetails, PaginatedResponse } from "@/@types/types";
    ...
    async function fetchPublishers() {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/publishers?key=${API_KEY}&page=${currentPage}&page_size=${pageSize}&ordering=name`
        );
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.detail ||
              `Failed to fetch publishers: ${response.statusText} (${response.status})`
          );
        }
        const data: PaginatedResponse<PublisherDetails> = await response.json();
        setPublishers(data.results);
        setTotalPublishers(data.count);
        setTotalPages(Math.ceil(data.count / pageSize));
      } catch (err) {
        if (err instanceof Error) setError(err.message);
        else setError("An unknown error occurred while fetching publishers.");
        setPublishers([]);
      } finally {
        setLoading(false);
      }
  ...
  if (loading && publishers.length === 0)
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-xl text-amber-400">Loading Game Publishers...</p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 py-8 px-4 sm:px-6 lg:px-8">
      ...
      {publishers.length > 0 ? (
          ...
          {publishers.map((publisher) => (
            <Link
              ...
              className="
                ...
                hover:scale-105 hover:shadow-amber-500/30 hover:border-amber-600
            ...
            >
              ...
              <div className="p-4">
                <h3 className="text-lg sm:text-xl font-semibold mb-1 truncate text-slate-100 group-hover:text-amber-300 transition-colors">
                  {publisher.name}
                </h3>
                <p className="text-sm text-slate-400 flex items-center">
                  <Gamepad2 size={14} className="mr-1.5 text-slate-500" />
                  Games: {publisher.games_count.toLocaleString()}
                </p>
              </div>
            </Link>
          ))}
        ...
        !loading && (
          <div className="text-center py-20">
            ...
            <p className="text-xl text-slate-400">No publishers found.</p>
        ...
        <div className="flex flex-col items-center space-y-4 mt-10">
          {loading && publishers.length > 0 && (
            <div className="flex items-center text-amber-400">
              ...
              <span>Loading more publishers...</span>
            </div>
          )}
          {totalPages > 1 && (
      ...
      <p className="text-center mt-10 text-sm text-slate-400">
        {totalPublishers > 0 &&
          `Showing ${
            publishers.length > 0 ? (currentPage - 1) * pageSize + 1 : 0
          }-${Math.min(
            currentPage * pageSize,
            totalPublishers
          )} of ${totalPublishers.toLocaleString()} publishers. `}
        Data from{" "}
        <a
          href="https://rawg.io"
          target="_blank"
